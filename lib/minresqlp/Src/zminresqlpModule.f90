...
module zminresqlpModule
  ...
  use  zminresqlpDataModule, only : dp, ip, one, zero, eps, realmin, zone, zzero, prcsn, debug
...
contains
  ...
  subroutine ZMINRESQLP( n, Aprod, b, shift, Msolve, disable, nout,       &
    ...
    ! if (nout_ > 0) write(nout_, tableHeaderStr)
    ...
    do while (istop_ <= flag0)
       ...
       alfa = real(dot_product(v,y), dp)      ! alphak
       y    = y + (- alfa/beta) * r2
       r1   = r2
       r2   = y

       if ( .not. precon_ ) then
          betan = znrm2(n, y, 1)                  ! betan = ||y||_2
       else
          call Msolve( n, r2, y )
          betan = real(dot_product(r2,y), dp)     ! betan = betak+1^2
          if (betan > zero) then
             betan = sqrt(betan)
          elseif ( znrm2(n, y, 1) > eps ) then    ! M must be indefinite.
             istop_ = 11
             exit
          end if
       end if
       ...
       if (itn_ == 1) then
          vec2(1)  = alfa
          vec2(2)  = betan
          pnorm    = znrm2(2, vec2, 1)
       else
          vec3(1) = beta
          vec3(2) = alfa
          vec3(3) = betan
          pnorm   = znrm2(3, vec3, 1)
       end if
       ...
       vec3(2) = ul2
       vec3(3) = ul
       xnorm_tmp = znrm2(3, vec3, 1)      !norm([xl2norm ul2 ul]);
       if (abs(gama) > zero  .and.  xnorm_tmp < maxxnorm_) then
          ...
          vec2(1) = xnorm_tmp
          vec2(2) = u
          if (likeLS  .and.  znrm2(2, vec2, 1) > maxxnorm_) then
             u      = zero
             istop_ = 12
          end if
       else
       ...
       vec2(1)   = xl2norm
       vec2(2)   = ul2
       xl2norm   = znrm2(2, vec2, 1)
       vec3(1)   = xl2norm
       vec3(2)   = ul
       vec3(3)   = u
       xnorm_    = znrm2(3, vec3, 1)
