...
void cg_solver::cg(const std::function<void(const Eigen::VectorXcd&,
    ...
    for (itn_ = 0; itn_ < max_iterations_; itn_++) {
        MPI_Allgatherv(p.data(), p.size(), MPI_DOUBLE_COMPLEX, gp.data(), lens,
        ...
        if (len) {
            la = p.dot(Ap);
            MPI_Allreduce(&la, &alpha, 1, MPI_DOUBLE_COMPLEX, MPI_SUM, nz_comm);
            alpha = rsold / alpha;
            lx += alpha * p;
            r -= alpha * Ap;
            lr = r.squaredNorm();
            MPI_Allreduce(&lr, &rsnew, 1, MPI_DOUBLE, MPI_SUM, nz_comm);
            abort = std::sqrt(rsnew) < rtol_;
        }
        if (!len || mpi::rank == 0) {
    ...
    if (len)
        MPI_Allgatherv(lx.data(), lx.size(), MPI_DOUBLE_COMPLEX, x.data(), lens,
