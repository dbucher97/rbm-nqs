...
cg_solver::cg_solver(size_t n, size_t m, int mloc, size_t n_neural,
...
void cg_solver::solve(const Eigen::MatrixXcd& mat, const Eigen::VectorXcd& d,
    ...
    Eigen::VectorXcd x1 = Eigen::Map<Eigen::VectorXcd>(x.data(), x.size());
    cg(Aprod, b, x1);
    ...
    //     std::cout << (A.diagonal() - diag).cwiseAbs().maxCoeff() <<
    //     std::endl; minresqlp_adapter min(mat_, d, r1, r2, rd, norm,
    //     n_neural_, diag_,
    //                           tmp_);
